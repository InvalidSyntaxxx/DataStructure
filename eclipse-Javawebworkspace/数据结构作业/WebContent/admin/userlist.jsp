...
<input class="layui-btn" type="button" onclick='location="http://localhost:8080/数据结构作业/admin/adduser.jsp"' value="新增"><%--有效！！！！ --%>
<input class="layui-btn" type="button" onclick='location="http://localhost:8080/数据结构作业/UserListServlet"' value="刷新"><%--有效！！！！ --%>
</div>
      ...
      <tbody>
        <c:forEach items="${list}" var="user">
            <tr>
               ...
               <td>
                  <a class="layui-btn layui-btn-primary layui-border-green" href ="${pageContext.request.contextPath }/GetUserServlet?id=${user.id}">修改</a>
                  <a class="layui-btn layui-btn-primary layui-border-red" href ="${pageContext.request.contextPath }/DeleteUserServlet?id=${user.id}" onclick="return confirm('确定删除这项吗？')">删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
</table>
		<div style="width:800px;font-size:20px;text-align:center">	    
		<c:choose>
